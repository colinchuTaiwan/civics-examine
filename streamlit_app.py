"""
civics_app.py — 公民測驗網頁（Streamlit Cloud + Firebase）
難度：國中、高中
每題限時：30 秒
"""
...
def save_record(name: str, score: int, difficulty: str) -> str:
    ...
    firebase_db.reference("records_civics").push({
        "id":         record_id,
        "name":       name,
        "score":      score,
...
def load_records_cached(difficulty: str) -> list:
    ...
    先行過濾，避免每次都把整個 records_civics 全部下載回來。
    需搭配 Firebase Rules 的 ".indexOn": ["difficulty"] 才有索引效能。
    """
    init_firebase()
    ref  = firebase_db.reference("records_civics")
...
st.set_page_config(page_title="公民測驗挑戰網", page_icon="⚖️", layout="centered")
...
.score-box   { font-size:3rem; font-weight:900; text-align:center; color:#b71c1c; margin:1rem 0; }
.champ-name  { font-size:1rem; font-weight:700; color:#b71c1c; }
...
if st.session_state.step == "login":
    st.markdown('<div class="big-title">⚖️ 公民測驗挑戰網</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-title">測試你的公民實力，挑戰榮譽榜！</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="visitor-badge">總瀏覽人次：{visitor_count} 次</div>',
                unsafe_allow_html=True)

    with st.form("login_form"):
        name      = st.text_input("請輸入你的名字：", placeholder="例如：大華")
...
# =========================
    ...
    with tab_quiz:
        ...
        tl = TIME_LIMITS.get(difficulty, 30)
        st.info(
        ...
        if not records:
            ...
            if not top10:
                ...
                for i, r in enumerate(top10):
                    ...
                    st.markdown(
                        f"{medal} &nbsp; **{r['name']}** &nbsp; "
                        f"<span style='color:#b71c1c;font-weight:700'>{r['score']} 分</span>"
